...
class _LeaderboardDialogState extends State<_LeaderboardDialog> {
  ...
  Widget build(BuildContext context) {
    ...
    // 路線2「固定畫布」：360 寬設計稿，小螢幕整張等比縮小 → 永不溢出。
    // Material：showDialog route 沒有 Material 祖先，IconButton/TextField 需要。
    return Material(
                    ...
                    Row(
                      children: [
                        ...
                        IconButton(
                          ...
                          icon: _gi('cancel', size: 20, color: _kTextDim),
                        ),
                      ],
                    ),
                    ...
                    Row(
                      children: [
                        ...
                        IconButton(
                          visualDensity: VisualDensity.compact,
                          onPressed: _editName,
                          icon: _gi('quill_ink', size: 16, color: _kGoldDeep),
                        ),
                      ],
                    ),
                  ],
  ...
  Future<void> _editName() async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => _themedDialog(
        ...
        accent: _kGold,
        title: '修改暱稱',
        // FittedBox 量測時寬度無界，TextField 必須有明確寬度（同結束彈窗）。
        extra: SizedBox(
          width: 220,
          child: TextField(
            controller: ctrl,
