...
///
/// 回傳被連到的敵人在 [positions] 內的索引，順序即實際連鎖（加入）順序。
/// [positions] 應只包含「可被連鎖的存活敵人」位置。
///
...
List<int> chainTargets({
  ...
  if (positions.isEmpty || limit <= 0) return const <int>[];
  ...
  final visited = <int>{};
  final order = <int>[];
  // 以落點 origin 作為起始節點，第一跳尋找其周圍的敵人。
  var frontier = <Vector2>[origin];
  ...
  while (frontier.isNotEmpty) {
    ...
    for (final node in frontier) {
      for (final c in candidates) {
        ...
        if (positions[c].distanceTo(node) > maxDistance) continue;
        visited.add(c);
        order.add(c);
        next.add(positions[c]);
      }
